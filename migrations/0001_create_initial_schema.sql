/*
  ...
  2. Seguridad
    ...
    - Se crean políticas para acceso autenticado
...
-- Políticas RLS para personas_casos
CREATE POLICY "Permitir lectura de personas_casos a usuarios autenticados"
  ON personas_casos FOR SELECT
  TO authenticated
  USING (TRUE);

CREATE POLICY "Permitir inserción de personas_casos a usuarios autenticados"
  ON personas_casos FOR INSERT
  TO authenticated
  WITH CHECK (TRUE);

CREATE POLICY "Permitir actualización de personas_casos a usuarios autenticados"
  ON personas_casos FOR UPDATE
  TO authenticated
  USING (TRUE)
  WITH CHECK (TRUE);

CREATE POLICY "Permitir eliminación de personas_casos a usuarios autenticados"
  ON personas_casos FOR DELETE
  TO authenticated
  USING (TRUE);

-- Políticas RLS para persona_telefonos
CREATE POLICY "Permitir lectura de persona_telefonos a usuarios autenticados"
  ON persona_telefonos FOR SELECT
  TO authenticated
  USING (TRUE);

CREATE POLICY "Permitir inserción de persona_telefonos a usuarios autenticados"
  ON persona_telefonos FOR INSERT
  TO authenticated
  WITH CHECK (TRUE);

CREATE POLICY "Permitir actualización de persona_telefonos a usuarios autenticados"
  ON persona_telefonos FOR UPDATE
  TO authenticated
  USING (TRUE)
  WITH CHECK (TRUE);

CREATE POLICY "Permitir eliminación de persona_telefonos a usuarios autenticados"
  ON persona_telefonos FOR DELETE
  TO authenticated
  USING (TRUE);

-- Políticas RLS para registros_comunicacion
CREATE POLICY "Permitir lectura de registros_comunicacion a usuarios autenticados"
  ON registros_comunicacion FOR SELECT
  TO authenticated
  USING (TRUE);

CREATE POLICY "Permitir inserción de registros_comunicacion a usuarios autenticados"
  ON registros_comunicacion FOR INSERT
  TO authenticated
  WITH CHECK (TRUE);

CREATE POLICY "Permitir actualización de registros_comunicacion a usuarios autenticados"
  ON registros_comunicacion FOR UPDATE
  TO authenticated
  USING (TRUE)
  WITH CHECK (TRUE);

CREATE POLICY "Permitir eliminación de registros_comunicacion a usuarios autenticados"
  ON registros_comunicacion FOR DELETE
  TO authenticated
  USING (TRUE);
